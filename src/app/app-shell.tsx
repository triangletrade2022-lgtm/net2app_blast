...
function LogsTab() {
  ...
  if(filter!=="all")params.set("connectionType",filter);
  if(q)params.set("search",q);
  if(dateFrom)params.set("from",dateFrom);
  if(dateTo)params.set("to",dateTo);
  const d=await api(`/api/sms/logs?${params}`);
  if(d&&d.logs){
    setLogs(d.logs);
    setTotal(d.total);
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-3 py-2 border-t border-gray-800">
          ...
          <div className="flex items-center gap-1">
            <button disabled={page <= 1} onClick={() => setPage(1)} className="px-2 py-1 rounded text-[10px] bg-gray-800 text-gray-400 hover:text-white disabled:opacity-30">First</button>
            <button disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="px-2 py-1 rounded text-[10px] bg-gray-800 text-gray-400 hover:text-white disabled:opacity-30">Prev</button>
            {visiblePages.map(p => (
              <button key={p} onClick={() => setPage(p)} className={"px-2 py-1 rounded text-[10px] " + (p === page ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:text-white")}>{p}</button>
            ))}
            <button disabled={page >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="px-2 py-1 rounded text-[10px] bg-gray-800 text-gray-400 hover:text-white disabled:opacity-30">Next</button>
            <button disabled={page >= totalPages} onClick={() => setPage(totalPages)} className="px-2 py-1 rounded text-[10px] bg-gray-800 text-gray-400 hover:text-white disabled:opacity-30">Last</button>
          </div>
        </div>
      )}
    </div>);
