...
function ClientsTab() {
  ...
  const [f, setF] = useState({ name:"",email:"",clientCode:"",alias:"",company:"",connectionType:"http",billingType:"on_submit",forceDlr:false,forceDlrStatus:"delivered",forceDlrTimeout:"0",maxTps:10,smppHost:"",smppSystemId:"",smppPassword:"",smppPort:2775,isActive:true,dlrCallbackUrl:"",creditLimit:"",currentBalance:"",allowedIps:"" });
  const load=useCallback(async()=>{const d=await api("/api/clients");if(Array.isArray(d))setItems(d);},[]);
  useEffect(()=>{load();},[load]);
  const reset=()=>{setF({name:"",email:"",clientCode:"",alias:"",company:"",connectionType:"http",billingType:"on_submit",forceDlr:false,forceDlrStatus:"delivered",forceDlrTimeout:"0",maxTps:10,smppHost:"",smppSystemId:"",smppPassword:"",smppPort:2775,isActive:true,dlrCallbackUrl:"",creditLimit:"",currentBalance:"",allowedIps:""});setEdit(null);setShow(false);};
  const sub=async(e:React.FormEvent)=>{e.preventDefault();const body:Record<string,unknown>={name:f.name,email:f.email,clientCode:f.clientCode||undefined,alias:f.alias||undefined,company:f.company||undefined,connectionType:f.connectionType,billingType:f.billingType,forceDlr:f.forceDlr,forceDlrStatus:f.forceDlrStatus,forceDlrTimeout:f.forceDlrTimeout||"0",maxTps:f.maxTps,isActive:f.isActive,dlrCallbackUrl:f.dlrCallbackUrl||undefined,creditLimit:f.creditLimit||undefined,currentBalance:f.currentBalance||undefined,allowedIps:f.allowedIps||undefined};if(f.connectionType==="smpp"){body.smppHost=f.smppHost;body.smppSystemId=f.smppSystemId;body.smppPassword=f.smppPassword;body.smppPort=f.smppPort;}if(edit){await api(`/api/clients/${edit.id}`,{method:"PUT",body:JSON.stringify(body)});}else{await api("/api/clients",{method:"POST",body:JSON.stringify(body)});}reset();load();};
  const del=async(id:number)=>{if(confirm("Delete?")){await api(`/api/clients/${id}`,{method:"DELETE"});load();}};
  const editC=(c:Client)=>{setEdit(c);setF({name:c.name,email:c.email,clientCode:c.clientCode||"",alias:c.alias||"",company:c.company||"",connectionType:c.connectionType,billingType:c.billingType||"on_submit",forceDlr:c.forceDlr,forceDlrStatus:c.forceDlrStatus||"delivered",forceDlrTimeout:c.forceDlrTimeout||"0",maxTps:c.maxTps||10,smppHost:c.smppHost||"",smppSystemId:c.smppSystemId||"",smppPassword:c.smppPassword||"",smppPort:c.smppPort||2775,isActive:c.isActive,dlrCallbackUrl:c.dlrCallbackUrl||"",creditLimit:c.creditLimit||"",currentBalance:c.currentBalance||"",allowedIps:c.allowedIps||""});setShow(true);};
  ...
  {show && (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      ...
      <form onSubmit={sub} className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div><label className="text-[10px] text-gray-500">Credit Limit</label><input type="number" step="0.0001" value={f.creditLimit} onChange={e=>setF({...f,creditLimit:e.target.value})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white"/></div>          <div><label className="text-[10px] text-gray-500">Initial Balance</label><input type="number" step="0.0001" value={f.currentBalance} onChange={e=>setF({...f,currentBalance:e.target.value})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white"/></div>
          <div className="md:col-span-3"><label className="text-[10px] text-gray-500">Allowed IPs (comma-separated, e.g. 1.2.3.4,192.168.1.0/24,10.0.0.*)</label><input value={f.allowedIps} onChange={e=>setF({...f,allowedIps:e.target.value})} placeholder="Leave empty for no IP restriction" className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white"/></div>
        <div className="flex items-end gap-4">
          ...
          <div><label className="text-[10px] text-gray-500">DLR Timeout (0=instant, N=seconds, 'random'=0-5s)</label><input value={f.forceDlrTimeout} onChange={e=>setF({...f,forceDlrTimeout:e.target.value})} placeholder="0" className="w-28 mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white"/></div>
  ...
  <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto">
    <table className="w-full text-xs">
      <thead><tr className="text-gray-500 border-b border-gray-800 text-left"><th className="p-3">Name</th><th className="p-3">Code</th><th className="p-3">Email</th><th className="p-3">Type</th><th className="p-3">Port</th><th className="p-3">Balance</th><th className="p-3">Billing</th><th className="p-3">DLR Timeout</th><th className="p-3">SMPP</th><th className="p-3">Status</th><th className="p-3">Actions</th></tr></thead><tbody>{items.length===0?<tr><td colSpan={11} className="p-8 text-center text-gray-600">No clients yet. Click "+ Add Client"</td></tr>:items.map(c=>(<tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30"><td className="p-3 font-medium">{c.name}</td><td className="p-3 font-mono text-gray-400">{c.clientCode||"-"}</td><td className="p-3 text-gray-400">{c.email}</td><td className="p-3"><span className={`px-2 py-0.5 rounded text-[10px] ${c.connectionType==="smpp"?"bg-purple-500/20 text-purple-400":"bg-cyan-500/20 text-cyan-400"}`}>{c.connectionType.toUpperCase()}</span></td><td className="p-3 text-gray-400">{c.connectionType==="smpp"?c.smppPort||"-":"-"}</td><td className="p-3 font-mono font-bold text-green-400">${parseFloat(c.currentBalance||"0").toFixed(4)}</td><td className="p-3 text-[10px]">{c.billingType==="on_dlr"?"On DLR":"On Submit"}</td><td className="p-3 font-mono text-[10px]">{c.forceDlrTimeout==="random"?<span className="text-purple-400">🎲 random</span>:c.forceDlrTimeout?`${c.forceDlrTimeout}s`:<span className="text-gray-600">-</span>}</td><td className="p-3">{c.connectionType==="smpp"?<Badge s={c.smppBindStatus==="bound"?"bound":"unbound"}/>:<span className="text-gray-600">-</span>}</td><td className="p-3"><Badge s={c.isActive?"Active":"Inactive"}/></td><td className="p-3 flex gap-2"><button onClick={()=>editC(c)} className="text-blue-400 hover:text-blue-300 font-medium">Edit</button><button onClick={()=>del(c.id)} className="text-red-400 hover:text-red-300">Delete</button></td></tr>))}</tbody></table></div></div>);
}
...
function SuppliersTab() {
  ...
  const [f, setF] = useState({ name:"",email:"",supplierCode:"",alias:"",company:"",connectionType:"http",apiUrl:"",apiKey:"",apiMethod:"GET",smppHost:"",smppSystemId:"",smppPassword:"",smppPort:2775,smppTls:false,smppBindType:"transceiver",senderId:"",forceDlr:false,isActive:true,priority:1 });
  const load=useCallback(async()=>{const d=await api("/api/suppliers");if(Array.isArray(d))setItems(d);},[]);
  useEffect(()=>{load();},[load]);
  const reset=()=>{setF({name:"",email:"",supplierCode:"",alias:"",company:"",connectionType:"http",apiUrl:"",apiKey:"",apiMethod:"GET",smppHost:"",smppSystemId:"",smppPassword:"",smppPort:2775,smppTls:false,smppBindType:"transceiver",senderId:"",forceDlr:false,isActive:true,priority:1});setEdit(null);setShow(false);};
  const sub=async(e:React.FormEvent)=>{e.preventDefault();const body:Record<string,unknown>={name:f.name,email:f.email,supplierCode:f.supplierCode||undefined,alias:f.alias||undefined,company:f.company||undefined,connectionType:f.connectionType,smppTls:f.smppTls,smppBindType:f.smppBindType,senderId:f.senderId||undefined,forceDlr:f.forceDlr,isActive:f.isActive,priority:f.priority};if(f.connectionType==="http"){body.apiUrl=f.apiUrl;body.apiKey=f.apiKey;body.apiMethod=f.apiMethod;}else{body.smppHost=f.smppHost;body.smppSystemId=f.smppSystemId;body.smppPassword=f.smppPassword;body.smppPort=f.smppPort;}if(edit){await api(`/api/suppliers/${edit.id}`,{method:"PUT",body:JSON.stringify(body)});}else{await api("/api/suppliers",{method:"POST",body:JSON.stringify(body)});}reset();load();};
  const del=async(id:number)=>{if(confirm("Delete?")){await api(`/api/suppliers/${id}`,{method:"DELETE"});load();}};
  const editS=(s:Supplier)=>{setEdit(s);setF({name:s.name,email:s.email,supplierCode:s.supplierCode||"",alias:s.alias||"",company:s.company||"",connectionType:s.connectionType,apiUrl:s.apiUrl||"",apiKey:s.apiKey||"",apiMethod:s.apiMethod||"GET",smppHost:s.smppHost||"",smppSystemId:s.smppSystemId||"",smppPassword:s.smppPassword||"",smppPort:s.smppPort||2775,smppTls:s.smppTls||false,smppBindType:s.smppBindType||"transceiver",senderId:s.senderId||"",forceDlr:s.forceDlr,isActive:s.isActive,priority:s.priority||1});setShow(true);};
  ...
  {show && (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      ...
      <form onSubmit={sub} className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        {f.connectionType==="smpp"&&<>
          <div><label className="text-[10px] text-gray-500">SMPP Host *</label><input required value={f.smppHost} onChange={e=>setF({...f,smppHost:e.target.value})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white focus:ring-1 focus:ring-blue-500 focus:outline-none"/></div>
          <div><label className="text-[10px] text-gray-500">System ID *</label><input required value={f.smppSystemId} onChange={e=>setF({...f,smppSystemId:e.target.value})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white focus:ring-1 focus:ring-blue-500 focus:outline-none"/></div>
          <div><label className="text-[10px] text-gray-500">Password *</label><input required value={f.smppPassword} onChange={e=>setF({...f,smppPassword:e.target.value})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white focus:ring-1 focus:ring-blue-500 focus:outline-none"/></div>
          <div><label className="text-[10px] text-gray-500">Port *</label><input required type="number" value={f.smppPort} onChange={e=>setF({...f,smppPort:parseInt(e.target.value)||2775})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white focus:ring-1 focus:ring-blue-500 focus:outline-none"/></div>
          <label className="flex items-center gap-2"><input type="checkbox" checked={f.smppTls} onChange={e=>setF({...f,smppTls:e.target.checked})} className="rounded"/><span className="text-[10px] text-gray-400">🔒 TLS</span></label>
          <div><label className="text-[10px] text-gray-500">Bind Type</label><select value={f.smppBindType} onChange={e=>setF({...f,smppBindType:e.target.value})} className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white"><option value="transceiver">TRX (Transceiver)</option><option value="transmitter">TX (Transmitter)</option><option value="receiver">RX (Receiver)</option></select></div>
          <div><label className="text-[10px] text-gray-500">Sender ID</label><input value={f.senderId} onChange={e=>setF({...f,senderId:e.target.value})} placeholder="Default sender ID for this supplier" className="w-full mt-0.5 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-xs text-white"/></div>
        </>}
  ...
  <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-gray-500 border-b border-gray-800 text-left"><th className="p-3">Name</th><th className="p-3">Code</th><th className="p-3">Type</th><th className="p-3">API/Host</th><th className="p-3">Port</th><th className="p-3">TLS</th><th className="p-3">Priority</th><th className="p-3">Bind</th><th className="p-3">Status</th><th className="p-3">Actions</th></tr></thead><tbody>{items.length===0?<tr><td colSpan={10} className="p-8 text-center text-gray-600">No suppliers. Click "+ Add Supplier"</td></tr>:items.map(s=>(<tr key={s.id} className="border-b border-gray-800/50 hover:bg-gray-800/30"><td className="p-3 font-medium">{s.name}</td><td className="p-3 font-mono text-gray-400">{s.supplierCode||"-"}</td><td className="p-3"><span className={`px-2 py-0.5 rounded text-[10px] ${s.connectionType==="smpp"?"bg-purple-500/20 text-purple-400":"bg-cyan-500/20 text-cyan-400"}`}>{s.connectionType.toUpperCase()}</span></td><td className="p-3 text-gray-400 max-w-[150px] truncate">{s.apiUrl||s.smppHost||"-"}</td><td className="p-3 text-gray-400">{s.connectionType==="smpp"?s.smppPort||"-":"-"}</td><td className="p-3">{s.connectionType==="smpp"?(s.smppTls?<span className="text-green-400">🔒</span>:<span className="text-gray-600">-</span>):<span className="text-gray-600">-</span>}</td><td className="p-3">{s.priority}</td><td className="p-3">{s.connectionType==="smpp"?<Badge s={s.smppBindStatus==="bound"?"bound":"unbound"}/>:<span className="text-gray-600">-</span>}</td><td className="p-3"><Badge s={s.isActive?"Active":"Inactive"}/></td><td className="p-3 flex gap-2"><button onClick={()=>editS(s)} className="text-blue-400 hover:text-blue-300 font-medium">Edit</button><button onClick={()=>del(s.id)} className="text-red-400 hover:text-red-300">Delete</button></td></tr>))}</tbody></table></div></div>);
}
...
function InvoicesTab() {
  ...
  const [detail,setDetail]=useState<{inv:Invoice;breakdown:{mcc:string;mnc:string;mccMnc:string;country:string;operator:string;totalSms:number;totalParts:number;rate:number;total:number}[];emailOpen?:boolean;emailTo?:string}|null>(null);
  ...
  const downloadPDF=async(inv:Invoice)=>{const blob=await api(`/api/invoices/${inv.id}/pdf`);if(blob instanceof Blob){const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`invoice-${inv.invoiceNumber}.pdf`;a.click();}};

  const showDetail=async(i:Invoice)=>{setLoadingDetail(i.id);try{const d=await api(`/api/invoices/${i.id}`);if(d&&d.invoiceData?.summary){setDetail({inv:i,breakdown:d.invoiceData.summary});}else{setDetail({inv:i,breakdown:[]});}}catch(e){console.error(e);setDetail({inv:i,breakdown:[]});}setLoadingDetail(null);};
  ...
  {detail&&(<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={()=>setDetail(null)}><div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-4xl w-full max-h-[85vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
    <div className="flex justify-between items-start mb-6"><div><h3 className="text-lg font-bold">{detail.inv.invoiceNumber}</h3><p className="text-xs text-gray-400 mt-1">{detail.inv.entityType==="client"?"👥":"🏢"} {detail.inv.entityName} | {new Date(detail.inv.periodStart).toLocaleDateString()} - {new Date(detail.inv.periodEnd).toLocaleDateString()}</p></div><button onClick={()=>setDetail(null)} className="text-gray-400 hover:text-white text-2xl">&times;</button></div>
    {detail.breakdown.length===0?<p className="text-gray-500 text-sm text-center py-8">No MCC-MNC breakdown data available for this invoice.</p>:<>
    <div className="overflow-x-auto mb-4"><table className="w-full text-xs"><thead><tr className="text-gray-500 border-b border-gray-700 text-left">
      <th className="p-2">MCC-MNC</th><th className="p-2">Country</th><th className="p-2">Operator</th><th className="p-2 text-right">SMS Count</th><th className="p-2 text-right">Parts</th>
      <th className="p-2 text-right">Rate/SMS</th><th className="p-2 text-right">{detail.inv.entityType==="client"?"Total Charge":"Total Billing"}</th>
    </tr></thead><tbody>{detail.breakdown.map((b,i)=>(<tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30">
      <td className="p-2 font-mono text-blue-400">{b.mccMnc||'N/A'}</td><td className="p-2">{b.country||'-'}</td><td className="p-2">{b.operator||'-'}</td><td className="p-2 text-right">{b.totalSms.toLocaleString()}</td><td className="p-2 text-right">{b.totalParts.toLocaleString()}</td>
      <td className="p-2 text-right text-green-400">${b.rate.toFixed(6)}</td><td className="p-2 text-right text-yellow-400 font-bold">${b.total.toFixed(4)}</td>
    </tr>))}</tbody></table></div>
    <div className="border-t border-gray-700 pt-3 flex flex-wrap gap-3 justify-between items-center text-xs">
      <div className="flex gap-2 items-center">
        <button onClick={()=>downloadPDF(detail.inv)} className="px-3 py-1.5 bg-red-600 text-white rounded text-xs hover:bg-red-500 font-medium">📥 Download PDF</button>
        {detail.emailOpen?<div className="flex gap-1 items-center"><input ref={el=>{if(el&&!detail.emailTo)el.value='';}} defaultValue={detail.emailTo||''} placeholder="recipient@email.com" className="w-44 px-2 py-1.5 bg-gray-800 border border-gray-700 rounded text-xs text-white" onChange={e=>{const v=e.target.value;setDetail(d=>({...d!,emailTo:v}));}}/><button onClick={async()=>{const res=await api(`/api/invoices/${detail.inv.id}/email`,{method:'POST',body:JSON.stringify({to:detail.emailTo||'',subject:`Invoice ${detail.inv.invoiceNumber} - ${detail.inv.entityName}`})});if(res.success){alert('Invoice sent!');setDetail(null);}else{alert(res.error||'Failed to send');}}} disabled={!detail.emailTo} className="px-2 py-1.5 bg-blue-600 text-white rounded text-xs disabled:opacity-50 hover:bg-blue-500">Send</button></div>:<button onClick={()=>setDetail(d=>({...d!,emailOpen:true}))} className="px-3 py-1.5 bg-blue-600 text-white rounded text-xs hover:bg-blue-500 font-medium">📧 Email</button>}
      </div>
      <div className="flex gap-4">
        <div><span className="text-gray-500">Total SMS:</span> <span className="font-bold">{detail.breakdown.reduce((a,b)=>a+b.totalSms,0).toLocaleString()}</span></div>
        <div><span className="text-gray-500">Total Parts:</span> <span className="font-bold">{detail.breakdown.reduce((a,b)=>a+b.totalParts,0).toLocaleString()}</span></div>
        <div><span className="text-gray-500">{detail.inv.entityType==="client"?"Total Charge":"Total Billing"}:</span> <span className="font-bold text-yellow-400">${detail.breakdown.reduce((a,b)=>a+b.total,0).toFixed(4)}</span></div>
      </div>
    </div>
    </>}
  </div></div>)}

  <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-gray-500 border-b border-gray-800 text-left"><th className="p-3">Inv#</th><th className="p-3">Type</th><th className="p-3">Entity</th><th className="p-3">Period</th><th className="p-3">Msgs</th><th className="p-3">Amount</th><th className="p-3">Breakdown</th><th className="p-3">Status</th><th className="p-3">Download</th><th className="p-3">Actions</th></tr></thead><tbody>{inv.length===0?<tr><td colSpan={10} className="p-6 text-center text-gray-600">No invoices</td></tr>:inv.map(i=>(<tr key={i.id} className="border-b border-gray-800/50"><td className="p-3 font-mono text-blue-400">{i.invoiceNumber}</td><td className="p-3"><span className={`px-2 py-0.5 rounded text-[10px] ${i.entityType==="client"?"bg-blue-500/20 text-blue-400":"bg-orange-500/20 text-orange-400"}`}>{i.entityType}</span></td><td className="p-3">{i.entityName}</td><td className="p-3 text-gray-400">{new Date(i.periodStart).toLocaleDateString()}-{new Date(i.periodEnd).toLocaleDateString()}</td><td className="p-3">{i.totalMessages?.toLocaleString()}</td><td className="p-3 text-green-400">${parseFloat(i.totalAmount||"0").toFixed(4)}</td><td className="p-3"><button onClick={()=>showDetail(i)} disabled={loadingDetail===i.id} className="px-2 py-0.5 rounded text-[10px] bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 disabled:opacity-50">{loadingDetail===i.id?"...":"📊 MCC-MNC"}</button></td><td className="p-3"><Badge s={i.status||"draft"}/></td><td className="p-3 flex gap-1"><button onClick={()=>downloadPDF(i)} className="text-red-400 text-xs font-medium hover:text-red-300">📥 PDF</button><button onClick={()=>downloadCSV(i)} className="text-green-400 text-xs">📥 CSV</button></td><td className="p-3 flex gap-1">{i.status==="draft"&&<button onClick={()=>up(i.id,"sent")} className="text-blue-400 text-xs">✉ Send</button>}{i.status==="sent"&&<button onClick={()=>up(i.id,"paid")} className="text-green-400 text-xs">✓ Paid</button>}</td></tr>))}</tbody></table></div></div>);
}
...
function TestSmsTab() {
  ...
  const smsInfo=useMemo(()=>{
    ...
    const maxSingle=isGsm?160:70;
    const maxMulti=isGsm?153:67;
    const len=txt.length;
    const parts=len<=maxSingle?1:Math.ceil(len/maxMulti);
