...
function SuppliersTab() {
  ...
  const [f, setF] = useState({ name:"",email:"",supplierCode:"",alias:"",company:"",connectionType:"http",apiUrl:"",apiKey:"",apiMethod:"GET",smppHost:"",smppSystemId:"",smppPassword:"",smppPort:2775,smppTls:false,forceDlr:false,isActive:true,priority:1 });
  const load=useCallback(async()=>{const d=await api("/api/suppliers");if(Array.isArray(d))setItems(d);},[]);
  useEffect(()=>{load();},[load]);
  const reset=()=>{setF({name:"",email:"",supplierCode:"",alias:"",company:"",connectionType:"http",apiUrl:"",apiKey:"",apiMethod:"GET",smppHost:"",smppSystemId:"",smppPassword:"",smppPort:2775,smppTls:false,forceDlr:false,isActive:true,priority:1});setEdit(null);setShow(false);};
  const sub=async(e:React.FormEvent)=>{e.preventDefault();const body:Record<string,unknown>={name:f.name,email:f.email,supplierCode:f.supplierCode||undefined,alias:f.alias||undefined,company:f.company||undefined,connectionType:f.connectionType,smppTls:f.smppTls,forceDlr:f.forceDlr,isActive:f.isActive,priority:f.priority};if(f.connectionType==="http"){body.apiUrl=f.apiUrl;body.apiKey=f.apiKey;body.apiMethod=f.apiMethod;}else{body.smppHost=f.smppHost;body.smppSystemId=f.smppSystemId;body.smppPassword=f.smppPassword;body.smppPort=f.smppPort;}if(edit){await api(`/api/suppliers/${edit.id}`,{method:"PUT",body:JSON.stringify(body)});}else{await api("/api/suppliers",{method:"POST",body:JSON.stringify(body)});}reset();load();};
  const del=async(id:number)=>{if(confirm("Delete?")){await api(`/api/suppliers/${id}`,{method:"DELETE"});load();}};
  const editS=(s:Supplier)=>{setEdit(s);setF({name:s.name,email:s.email,supplierCode:s.supplierCode||"",alias:s.alias||"",company:s.company||"",connectionType:s.connectionType,apiUrl:s.apiUrl||"",apiKey:s.apiKey||"",apiMethod:s.apiMethod||"GET",smppHost:s.smppHost||"",smppSystemId:s.smppSystemId||"",smppPassword:s.smppPassword||"",smppPort:s.smppPort||2775,smppTls:s.smppTls||false,forceDlr:s.forceDlr,isActive:s.isActive,priority:s.priority||1});setShow(true);};
