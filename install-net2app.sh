...
# After any future edit to a .java source under java-smsc-gateway/src/main/java,
# run `bash scripts/rebuild-smsc.sh` to rebuild the JAR and restart net2app-smsc.
# `mvn compile` alone is NOT enough — the deployed JAR is only regenerated by
# `mvn package` (which this script does automatically).
chmod +x "${APP_DIR}/scripts/rebuild-smsc.sh" 2>/dev/null || true
echo -e "${GREEN}  ↪ To rebuild the SMSC JAR + restart pm2 after editing Java sources:${NC}"
echo -e "${GREEN}    bash ${APP_DIR}/scripts/rebuild-smsc.sh${NC}"
...
# Make sure pm2's systemd unit was actually written (failure was previously silently swallowed).
PM2_STARTUP_OUT="$(env PATH=$PATH:/usr/bin pm2 startup systemd -u "${APP_USER}" --hp "/home/${APP_USER}" 2>&1)"
STARTUP_RC=$?
echo "${PM2_STARTUP_OUT}"
if [[ ${STARTUP_RC} -ne 0 ]]; then
    echo -e "${RED}WARN: pm2 startup failed (rc=${STARTUP_RC}). Services will NOT auto-start after reboot.${NC}"
else
    # Verify the systemd unit file exists
    UNIT_FILE="/etc/systemd/system/pm2-${APP_USER}.service"
    if [[ -f "${UNIT_FILE}" ]]; then
        systemctl enable "pm2-${APP_USER}.service" 2>/dev/null || true
        # Verify systemd actually accepted the unit so we don't lie on the green tick line.
        if systemctl is-enabled --quiet "pm2-${APP_USER}.service" 2>/dev/null; then
            echo -e "${GREEN}✓ pm2 systemd unit installed and enabled: ${UNIT_FILE}${NC}"
        else
            echo -e "${RED}WARN: pm2 unit file exists but systemctl is-enabled returned non-zero. Auto-restart after reboot is NOT guaranteed.${NC}"
            systemctl status "pm2-${APP_USER}.service" 2>&1 | head -20 || true
        fi
    else
        echo -e "${RED}WARN: pm2 startup reported success but no systemd unit file was written at ${UNIT_FILE}${NC}"
    fi
fi
